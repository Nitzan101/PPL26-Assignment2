...
export const isClassExp = (x: any): x is ClassExp => x.tag === "ClassExp"; //Q2a
...
const parseLetExp = (bindings: Sexp, body: Sexp[]): Result<LetExp> => {
    ...
    const vars = map((b: [string, Sexp]) => b[0], bindings);
...
export const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> => {
    ...
    const vars = map((b: [string, Sexp]) => b[0], methods);
    ...
    return mapv(methodsResult, (methods) => makeClassExp(parsedFields, methods)
    );
}
